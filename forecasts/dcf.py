...
coe = pd.read_excel(config.FORECAST_DATA, sheet_name='COE', index_col=0, usecols=['Ticker', 'COE'], engine='openpyxl')
...
required_cols = [
    "Revenue", "EPS", "OCF", "InterestAfterTax", "Capex",
    "EBIT", "Depreciation & Amortization", "Share-Based Compensation",
    "Acquisitions", "Net Income", "EBITDA",
    "Change in Working Capital", "Other Operating Activities", "FCF"
]
...
for ticker in tickers:
    ...
    lb = config.LBR * latest_prices_series[ticker] 
    ub = config.UBR * latest_prices_series[ticker]
