...
capm_table = pd.read_excel(EXCEL_OUT_FILE, header=0, sheet_name='CAPM Pred', index_col=0, na_values=0)
coe = pd.Series({ticker: capm_table.loc[ticker, 'Returns'] for ticker in tickers})
capm_bl_table = pd.read_excel(EXCEL_OUT_FILE, header=0, sheet_name='CAPM BL Pred', index_col=0, na_values=0)
coe_b = pd.Series({ticker: capm_bl_table.loc[ticker, 'Returns'] for ticker in tickers})
r_dicts = r.dicts()
...
funcs = [constrained_regression, constrained_regression, constrained_regression,
            ordinary_regression, ordinary_regression, ordinary_regression,
            constrained_regression, constrained_regression, constrained_regression,
            ordinary_regression, ordinary_regression]

def _zero_dicts(ticker):
    for d in (low_price_dict, avg_price_dict, high_price_dict, returns_dict, se_dict):
        d[ticker] = 0

low_price_dict, avg_price_dict, high_price_dict = {}, {}, {}
returns_dict, se_dict = {}, {}
...
for ticker in tickers:
    ...
    regression_data = fin_df.dropna(subset=["Revenue", "EPS", "OCF", "InterestAfterTax", "Capex",
                                             "EBIT", "Depreciation & Amortization", "Share-Based Compensation",
                                             "Acquisitions", "Net Income", "EBITDA", "Change in Working Capital",
                                             "Other Operating Activities", "FCF"])
    ...
    y_series = {col: regression_data[col].values for col in ["OCF", "InterestAfterTax",
                                                             "EBIT", "Depreciation & Amortization",
                                                             "Share-Based Compensation", "Acquisitions",
                                                             "Net Income", "FCF", "EBITDA",
                                                             "Change in Working Capital", "Other Operating Activities"]}
    ...
    coe_bl = coe_b.clip(lower=rf)[ticker]
    ...
    WACC = (coe_bl * E / V) + (cost_of_debt * mv_debt / V)
    ...
    fcff_raw = np.stack([
        np.maximum(ocf_p + int_p - capex, 0),
        np.maximum(ebit_p + da_p + sbc_p + ooa_p + aq_p - cwc_p - capex, 0),
        np.maximum(ni_p + int_p + da_p + sbc_p + ooa_p + aq_p - cwc_p - capex, 0),
        np.maximum(fcf_p + int_p, 0),
        np.maximum(ebitda_p - capex + aq_p - cwc_p, 0)
    ], axis=0) 
    ...
    lb = 0.2 * latest_prices_series[ticker]; ub = 5 * latest_prices_series[ticker]
