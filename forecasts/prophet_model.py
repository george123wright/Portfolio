...
def clip_to_bounds(df: pd.DataFrame, price: float) -> pd.DataFrame:
    """
...
def evaluate_forecast(model: Prophet, initial: str, period: str, horizon: str) -> pd.DataFrame:
    """
...
def _linear(series: pd.Series, end_value: Union[float, np.nan]) -> pd.Series:
    """
...
def forecast_with_prophet(
    ...
    if 'EPS (Basic)' in regressors:
        if not pd.isna(eps_target):
            future.loc[horizon_mask, 'EPS (Basic)'] = _linear(
                future.loc[horizon_mask, 'EPS (Basic)'],
                eps_target
            )
    ...
    if interp_int_allH is not None and 'Interest' in regressors:
        ...
        future.loc[horizon_mask, 'Interest'] = _linear(
            ...
            float(interp_int_allH) if np.isscalar(interp_int_allH) else None
        )
    ...
    if interp_inf_allH is not None and 'Cpi' in regressors:
        ...
        future.loc[horizon_mask, 'Cpi'] = _linear(
            ...
            float(interp_inf_allH) if np.isscalar(interp_inf_allH) else None
        )
    ...
    if interp_gdp_allH is not None and 'Gdp' in regressors:
        ...
        future.loc[horizon_mask, 'Gdp'] = _linear(
            ...
            float(interp_gdp_allH) if np.isscalar(interp_gdp_allH) else None
        )
    ...
    if interp_unemp_allH is not None and 'Unemp' in regressors:
        ...
        future.loc[horizon_mask, 'Unemp'] = _linear(
            ...
            float(interp_unemp_allH) if np.isscalar(interp_unemp_allH) else None
        )
...
def main() -> None:
    ...
    for ticker in tickers:
        ...
        try:
            m_prophet = prepare_prophet_model(df_model[['ds', 'y'] + regressors], regressors)
        except Exception as e:
            logger.error("Failed to fit Prophet model for %s: %s", ticker, e)
            min_price[ticker] = max_price[ticker] = avg_price[ticker] = scenario_se[ticker] = avg_returns_dict[ticker] = 0.0
            continue

        cv_metrics = evaluate_forecast(m_prophet, initial=cv_initial, period=cv_period, horizon=cv_horizon)
