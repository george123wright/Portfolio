...
class FinancialForecastData:
    ...
    def currency_adjustment(self, ticker: str, df: pd.Series | pd.DataFrame):
        ex = self.macro.currency
        if ticker in ["BABA","PDD"]:
            return df * ex.loc["USDCNY"]
        if ticker in ["BP.L","HSBA.L"]:
            return df * 10 / ex.loc["GBPUSD"]
        if ticker == "ASX":
            return df * 30
        return df
    ...
    def _read_sheet(self, tkr: str, sheet: str, rows: list[str]) -> pd.DataFrame:
    ...
    def income(self) -> dict[str,pd.DataFrame]:
        ...
        for t in self.tickers:
            try:
                d[t] = self._read_sheet(t, self._SHEET_INCOME, self._INCOME_ROWS)
            except Exception as e:
                if not self.quiet: print(f"[WARN] income {t}: {e}")
                d[t] = pd.DataFrame()
        return d
    ...
    def cash(self) -> dict[str,pd.DataFrame]:
        ...
        for t in self.tickers:
            try:
                ...
                if not self.quiet: print(f"[WARN] cash {t}: {e}")
                d[t] = pd.DataFrame()
        return d
    ...
    def bal(self) -> dict[str,pd.DataFrame]:
        ...
        for t in self.tickers:
            try:
                ...
                if not self.quiet: print(f"[WARN] bal {t}: {e}")
                d[t] = pd.DataFrame()
        return d
    ...
    def ratios(self) -> dict[str,pd.DataFrame]:
        ...
        for t in self.tickers:
            try:
                d[t] = self._read_sheet(t, self._SHEET_RATIOS, self._RATIOS_ROWS)
            except Exception as e:
                if not self.quiet: print(f"[WARN] ratios {t}: {e}")
                d[t] = pd.DataFrame()
        return d
    ...
    def ratios_ann(self) -> dict[str,pd.DataFrame]:
        ...
        for t in self.tickers:
            try:
                d[t] = self._read_sheet(t, self._SHEET_RATIOS_ANN, self._RATIOS_ROWS_ANN)
            except Exception as e:
                if not self.quiet: print(f"[WARN] ratios_ann {t}: {e}")
                d[t] = pd.DataFrame()
        return d
    ...
    def income_ann(self) -> dict[str,pd.DataFrame]:
        ...
        for t in self.tickers:
            try:
                d[t] = self._read_sheet(t, self._SHEET_INCOME_ANN, self._INCOME_ROWS_ANN)
            except Exception as e:
                if not self.quiet: print(f"[WARN] income_ann {t}: {e}")
                d[t] = pd.DataFrame()
        return d
    ...
    def annuals(self) -> dict[str,pd.DataFrame]:
        ...
        for t in self.tickers:
            try:
                ...
                rev_hist = self.currency_adjustment(t, inc.loc["Revenue"])
                ...
                ocf   = self.currency_adjustment(t, cf.loc["Operating Cash Flow"])
                capex = self.currency_adjustment(t, cf.loc["Capital Expenditures"])
                aq    = -self.currency_adjustment(t, cf.loc["Acquisitions"])
                fcf   = self.currency_adjustment(t, cf.loc["Free Cash Flow"])
                sbc   = self.currency_adjustment(t, cf.loc["Share-Based Compensation"])
                ooa   = self.currency_adjustment(t, cf.loc["Other Operating Activities"])
                div   = self.currency_adjustment(t, cf.loc["Dividends Paid"])

                wc      = self.currency_adjustment(t, bal.loc["Working Capital"])
                tot_d   = self.currency_adjustment(t, bal.loc["Total Debt"])
                bvps_0  = self.currency_adjustment(t, bal.loc["Book Value Per Share"].iat[-1])

                ebit   = self.currency_adjustment(t, inc.loc["EBIT"])
                da     = self.currency_adjustment(t, inc.loc["Depreciation & Amortization"])
                ni     = self.currency_adjustment(t, inc.loc["Net Income"])
                ebitda = self.currency_adjustment(t, inc.loc["EBITDA"])
                tax    = self.currency_adjustment(t, inc.loc["Income Tax"])
                pretax = self.currency_adjustment(t, inc.loc["Pretax Income"])
                iexp   = self.currency_adjustment(t, inc.loc["Interest Expense / Income"])
                ...
                if not self.quiet: print(f"[WARN] annuals {t}: {e}")
                out[t] = pd.DataFrame()
        return out


    def _read_analyst_sheet(self, path: Path, tkr: str) -> pd.DataFrame:
    ...
    def _make_synthetic_forecast(
        ...
        rev_g   = self._filter_outliers_iqr(ann_num.loc["Revenue Growth"].dropna())
        eps_g   = self._filter_outliers_iqr(ann_num.loc["EPS Growth"].dropna())
    ...
    def forecast(self) -> dict[str,pd.DataFrame]:
        ...
        for t in self.tickers:
            try:
                ...
                if fpath.exists():
                    df = self._read_analyst_sheet(fpath, t)
                else:
                    ann_df = self.income_ann[t]
                    rev    = self.annuals[t]["Revenue"]
                    eps    = self.annuals[t]["EPS"]
                    df     = self._make_synthetic_forecast(t, rev, eps, ann_df)
                out[t] = df
            except Exception as e:
                if not self.quiet: print(f"[WARN] forecast {t}: {e}")
    ...
    def kpis(self) -> dict[str,pd.DataFrame]:
        ...
        for t in self.tickers:
            try:
                ...
                if not self.quiet: print(f"[WARN] kpis {t}: {e}")
                out[t] = self._empty_kpis()
        return out

    def _compute_kpis(
        self, tkr, ratios_df, capex, rev, annual_index, bvps_0
    ) -> pd.DataFrame:
        ...
        pe_a   = pe.ewm(span=20,adjust=False).mean().iat[-1]
        ps_a   = ps.ewm(span=20,adjust=False).mean().iat[-1]
        ptb_a  = ptb.ewm(span=20,adjust=False).mean().iat[-1]

        mc = ratios_df.loc["Market Capitalization"].apply(_parse_abbrev)
        ev = ratios_df.loc["Enterprise Value"].apply(_parse_abbrev)
        mc_ev = (mc/ev).replace([np.inf,-np.inf],np.nan).ewm(span=20,adjust=False).mean().iat[-1]
    ...
    def prophet_data(self) -> dict[str,pd.DataFrame]:
        ...
        for t in self.tickers:
            try:
                ...
                rev = self.currency_adjustment(t, inc2.loc["Revenue"].ffill().fillna(0))
                eps = self.currency_adjustment(
                    t, pd.to_numeric(inc2.loc["EPS (Basic)"], errors="coerce").ffill().fillna(0)
                )
                out[t] = pd.DataFrame({"rev":rev, "eps":eps})
            except Exception as e:
                if not self.quiet: print(f"[WARN] prophet_data {t}: {e}")
                out[t] = pd.DataFrame()
        return out
