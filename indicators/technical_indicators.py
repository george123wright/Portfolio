...
def persistent_position(buy: pd.Series, sell: pd.Series) -> pd.Series:
    """
...
def mfi_signals(high: pd.Series, low: pd.Series, close: pd.Series, volume: pd.Series, *, window: int = MFI_WINDOW, buy_thresh: float = MFI_BUY_THRESH, sell_thresh: float = MFI_SELL_THRESH) -> Tuple[pd.Series, pd.Series]:
    """
...
def vwap_signals(high: pd.Series, low: pd.Series, close: pd.Series, volume: pd.Series, *, window: int = VWAP_WINDOW) -> Tuple[pd.Series, pd.Series]:
...
def score_for_ticker(data: dict[str, pd.DataFrame], tk: str) -> pd.Series:
    ...
    adx_filter = (adx_series(h, l, c, window=ADX_WINDOW) > ADX_THRESHOLD).fillna(False)
    ...
    for name, fn in indicators.items():
        buy, sell = fn()
        pos = persistent_position(buy, sell)
