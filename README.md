...
* **`financial_data.py`** – Retrieves analyst estimates and financial statements
  ...
  revenue growth and analyst price targets. It assigns a standard error metric for the average price prediction via the equation (Max Price Prediction - Min Price Prediction) / (2 * Z-score * Price) where alpha for the Z-score is calculated using alpha 1 / number of analysts.
